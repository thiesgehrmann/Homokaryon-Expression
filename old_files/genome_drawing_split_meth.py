...
def draw_genome(chromosome_list, ideogram, ideogene, ideometh, outfig, ideogroup=None):
  ...
  for chrom in chromosome_list[::-1]:
      ...
      gene_red_ybase[chrom] = ybase+gene_height
      gene_blue_ybase[chrom] = ybase
      meth_ybase[chrom] = ybase - group_height - group_padding
  ...
  genes_red = genes[genes['mean'] >= 1.0]
  genes_blue = genes[genes['mean'] < 1.0]

  meth = ideometh;
  meth.columns = ['chrom', 'start'];
  meth = meth[meth.chrom.apply(lambda x: x in chromosome_list)]
  meth['width'] = 3000
  meth['start'] = meth['start'] - 1500;
  meth['colors'] = meth.start.apply(lambda x: (0, 1, 0));
  ...
  for collection in chromosome_collections(ideo, chrom_ybase, chrom_height, linewidth=1.0):
      ax.add_collection(collection)
 
  # ...and the gene data
  print("adding genes...")
  for collection in chromosome_collections(genes_red, gene_red_ybase, gene_height, alpha=0.5, linewidths=0.05):
      ax.add_collection(collection)

  for collection in chromosome_collections(genes_blue, gene_blue_ybase, gene_height, alpha=0.5, linewidths=0.05):
      ax.add_collection(collection)

  for collection in chromosome_collections(meth, meth_ybase, group_height, alpha=1.0, linewidths=0.00):
      ax.add_collection(collection)
